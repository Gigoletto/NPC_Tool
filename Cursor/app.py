...
def _theme_override_css() -> str:
    ...
    return (
        ...
        "[role='checkbox'][data-selected='true']{"
        f"accent-color:{primary}!important;"
        f"border-color:{primary}!important;"
        "}"
...
def _inject_theme_into_parent() -> None:
    ...
    script = (
        ...
        "function isRed(c){"
        "var s=String(c||'');"
        "if(/#ff4b4b|#f63366|#ff2b2b|#ff4b4b|#e74c3c/i.test(s))return true;"
        "var m=s.match(/rgba?\\(\\s*(\\d+)\\s*[,\\s]\\s*(\\d+)\\s*[,\\s]\\s*(\\d+)/i);"
        "if(m){var r=+m[1],g=+m[2],b=+m[3];return r>=200&&g<=110&&b<=110&&r>g+70&&r>b+70;}"
        "return false;"
        "}"
        ...
        "'[data-testid=stRadio] [role=radio],[data-testid=stRadio] label,"
        +"'[data-testid=stCheckbox] [role=checkbox],[data-testid=stCheckbox] label,"
        +"'[data-baseweb=radio],[data-baseweb=checkbox],[role=radio],[role=checkbox]'"
        ...
        +"||(item.getAttribute('data-selected')!=null&&item.getAttribute('data-selected')!=='false')"
        +"||(input&&input.checked);"
        "if(!on)return;"
        "item.querySelectorAll('div,span').forEach(function(el){"
        "var cs=view.getComputedStyle(el);"
        "var box=el.getBoundingClientRect();"
        "if(box.width<8||box.width>36||box.height<8||box.height>36)return;"
        "if(!(isRed(cs.backgroundColor)||isRed(cs.borderColor)||isRed(cs.borderTopColor)))return;"
        "el.style.setProperty('background-color','#00E5FF','important');"
        "el.style.setProperty('border-color','#00E5FF','important');"
        "el.style.setProperty('filter','none','important');"
        "});"
        ...
        "}"
        "paint();"
        "if(!doc.documentElement.__sr5ThemeWatch3){"
        "doc.documentElement.__sr5ThemeWatch3=true;"
